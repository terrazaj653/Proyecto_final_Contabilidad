...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Bind Ledger Auxiliar interactivity
    const mayorSelect = document.getElementById("mayor-account-select");
    if (mayorSelect) {
        mayorSelect.addEventListener("change", (e) => {
            renderLibroMayorTable(parseInt(e.target.value));
        });
    }
    
    // Bind Balance General exports
    const btnBgExcel = document.getElementById("btn-bg-export-excel");
    if (btnBgExcel) btnBgExcel.addEventListener("click", () => {
        tableToCSV("balance-general-table", "Balance_General.csv");
    });
    
    const btnBgWord = document.getElementById("btn-bg-export-word");
    if (btnBgWord) btnBgWord.addEventListener("click", () => {
        tableToWord("balance-general-table", "Balance_General.doc", "Balance General");
    });
    
    const btnBgPDF = document.getElementById("btn-bg-export-pdf");
    if (btnBgPDF) btnBgPDF.addEventListener("click", exportBGToPDF);
...
function renderLibroMayor(ledger) {
    const select = document.getElementById("mayor-account-select");
    if (!select) return;
    
    const previouslySelected = select.value;
    
    // Re-populate selector (in case catalog accounts were added/removed/renamed)
    select.innerHTML = "";
    const sortedCatalog = getSortedCatalog();
    sortedCatalog.forEach(acct => {
        const opt = document.createElement("option");
        opt.value = acct.code;
        opt.textContent = `[${acct.code}] ${acct.name} (${acct.type})`;
        select.appendChild(opt);
    });
    
    // Restore selection or pick the first one
    if (previouslySelected && sortedCatalog.some(a => a.code == previouslySelected)) {
        select.value = previouslySelected;
    } else if (sortedCatalog.length > 0) {
        select.value = sortedCatalog[0].code;
    }
    
    // Render the table details for the selected account
    const selectedCode = parseInt(select.value);
    if (selectedCode) {
        renderLibroMayorTable(selectedCode);
    }
}

function renderLibroMayorTable(selectedCode) {
    const tbody = document.getElementById("mayor-table-body");
    if (!tbody) return;
    tbody.innerHTML = "";
    
    const acct = catalog.find(c => c.code === selectedCode);
    if (!acct) return;
    
    const sortedAllEntries = [...entries].sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    sortedAllEntries.forEach((entry, entryIdx) => {
        const partidaNum = entryIdx + 1;
        const matchingDetails = entry.details.filter(d => d.code === selectedCode);
        
        matchingDetails.forEach(det => {
            const debe = det.debe || 0;
            const haber = det.haber || 0;
            ...
            const dateObj = new Date(entry.date + "T00:00:00");
            const formattedDate = dateObj.toLocaleDateString("es-GT", { day: 'numeric', month: '2-digit', year: 'numeric' });
            
            rowsHtml += `
                <tr>
                    <td>${formattedDate}</td>
                    <td><strong>Partida #${partidaNum}</strong></td>
                    <td>${entry.description}</td>
                    <td class="monto debit-val">${debe > 0 ? 'Q' + debe.toFixed(2) : '-'}</td>
                    <td class="monto credit-val">${haber > 0 ? 'Q' + haber.toFixed(2) : '-'}</td>
                    <td class="monto" style="font-weight: bold; color: ${runningBalance >= 0 ? 'var(--color-success)' : 'var(--color-danger)'};">
                        Q${runningBalance.toFixed(2)}
                    </td>
                </tr>
            `;
        });
    ...
    if (rowsHtml === "") {
        ...
        // Add footer total row dynamically if there are movements
        const tfoot = document.createElement("tr");
        tfoot.className = "total-row";
        tfoot.innerHTML = `
            <td colspan="3" style="text-align: right; padding-right: 15px; font-weight: bold;">Totales Acumulados:</td>
            <td class="monto debit-val" style="font-weight: bold;">Q${totalDebe.toFixed(2)}</td>
            <td class="monto credit-val" style="font-weight: bold;">Q${totalHaber.toFixed(2)}</td>
            <td class="monto" style="font-weight: bold; color: ${runningBalance >= 0 ? 'var(--color-success)' : 'var(--color-danger)'};">
                Q${runningBalance.toFixed(2)}
            </td>
        `;
...
function tableToWord(tableId, filename, titleText) {
    const table = document.getElementById(tableId);
    if (!table) return;
...
function exportBGToPDF() {
    const printDate = document.getElementById("print-bg-date");
    if (printDate) printDate.textContent = new Date().toLocaleDateString("es-GT", { day: 'numeric', month: 'long', year: 'numeric' });
    window.print();
}
